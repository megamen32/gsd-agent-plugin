...
- **Global scope**: `/tmp/gsd-npm-codex-stage/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/tmp/gsd-npm-codex-stage/notes/`)
...
2. Glob `/tmp/gsd-npm-codex-stage/notes/*.md` (if directory exists) — global notes
...
Global (/tmp/gsd-npm-codex-stage/notes/):
...
2. **No `.planning/`**: Falls back to global `/tmp/gsd-npm-codex-stage/notes/` — works in any directory
